...
void computeFunction(void* computeDataStruct){                                       //function that computes data, takes compute struct as input
    ...
    tempComputedData.push(5 + (0.75 * (tempRawData.shift())));         //convert raw temp and point pTempComputed pointer to it
    sysComputedData.push(9 + (2    * (sysRawData.shift())));       //convert raw sys and point pSystolicPressComputed pointer to it
    diaComputedData.push(6 + (1.5  * (diaRawData.shift())));      //convert raw dia and point pDiastolicPressComputed pointer to it
    pulseComputedData.push(8 + (3    * (pulseRawData.shift())));           //convert raw pulse and point pPulseRateComputed pointer to it
}
