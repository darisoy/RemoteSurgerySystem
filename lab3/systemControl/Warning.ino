...
void alarmFunction(void* warningStruct){                                                             //function that checks if alarms should be going off, takes in the warning struct
  ...
  wData->pBloodPressCorrectedBuf.endList = &diastolicCorrectedBuffer;                                     //assign corrected fia's address to corrected dia pointer from warning struct
  wData->pPulseRateCorrected      = &pulseRateCorrectedBuffer;                                             //assign corrected pulse's address to corrected pulse pointer from warning struct
  ...
  diaGoodBool     = (BufferRead(wData->pBloodPressCorrectedBuf.endList) == 80);                                        //if value of the corrected dia pointer is 80, make the boolean true, otherwise false
  prGoodBool      = (BufferRead(wData->pPulseRateCorrected) > 60.0) && (BufferRead(wData->pPulseRateCorrected) < 100.0);   //if value of the corrected pulse rate pointer is greater than 60 and less than 100, make the boolean true, otherwise false
